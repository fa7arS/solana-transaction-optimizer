...
const lookupTableAddressCreation = async () => {
  ...
  try {
    ...
    console.log(
      `[SUCCESS] Lookup table extended successfully with ${accounts.length} accounts`
    );
...
const buyWithLookup = async () => {
  ...
  try {
    ...
    if (tx1.success) {
      ...
      console.log(
        `[DATA] Lookup table contains ${lookupTableAccount.state.addresses.length} addresses`
      );
...
const LookupImprovement = async () => {
  ...
  try {
    ...
    if (
      ...
      console.log(
        `   Compression Ratio:      ${(
          normalResult.size / lookupResult.size
        ).toFixed(2)}:1`
      );
      ...
      if (lookupResult.lookupTableAccounts) {
        console.log(
          `   Lookup Table Accounts:  ${lookupResult.lookupTableAccounts}`
        );
      }
      ...
      if (parseFloat(percentReduction) > 50) {
        console.log(
          `   EXCELLENT: ${percentReduction}% reduction is highly effective!`
        );
      } else if (parseFloat(percentReduction) > 30) {
        console.log(
          `   GOOD: ${percentReduction}% reduction provides solid benefits`
        );
      } else if (parseFloat(percentReduction) > 15) {
        console.log(`   MODERATE: ${percentReduction}% reduction is decent`);
      } else {
        console.log(
          `   MINIMAL: ${percentReduction}% reduction may not justify complexity`
        );
      }
      ...
      console.log(
        `   Benefits: Better network efficiency, reliability, and future-proofing`
      );

      console.log(`\nNETWORK IMPACT:`);
      const bandwidthSaved = (byteSavings / 1024).toFixed(2);
      console.log(
        `   Bandwidth Saved:        ${bandwidthSaved} KB per transaction`
      );
      console.log(
        `   Block Space Efficiency: ${percentReduction}% more efficient`
      );
      ...
      if (parseFloat(percentReduction) > 40) {
        console.log(
          `   [RECOMMENDED] Highly recommend using lookup tables for this use case`
        );
        console.log(
          `   [RECOMMENDED] Consider implementing for similar transaction patterns`
        );
      } else if (parseFloat(percentReduction) > 20) {
        console.log(
          `   [RECOMMENDED] Lookup tables provide good value for this use case`
        );
        console.log(
          `   [SUGGESTION] Monitor for additional optimization opportunities`
        );
      } else {
        console.log(
          `   [CAUTION] Evaluate if lookup table complexity is worth the savings`
        );
        console.log(
          `   [SUGGESTION] Consider batching more operations to increase efficiency`
        );
      }
    } else {
      console.log("\nCOMPARISON FAILED:");
      if (!normalResult.success) {
        console.log(
          `   Normal transaction error: ${
            normalResult.error || "Unknown error"
          }`
        );
      }
      if (!lookupResult.success) {
        console.log(
          `   Lookup transaction error: ${
            lookupResult.error || "Unknown error"
          }`
        );
      }
    }
